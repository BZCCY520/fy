...
- Emby 接入：登录 Emby 服务器、浏览媒体库视频，并载入到应用内播放器进行识别翻译。
...
- Emby 使用方式：在视频听译页点击 `连接 Emby`，填写 `http://服务器IP:8096`、用户名和密码；登录后点击 `Emby 视频` 选择媒体库视频，再点击 `开始视频听译`。
- 如果 Emby 在局域网内且使用 HTTP，iOS 真机会请求本地网络权限；已在 `Info.plist` 中开启本地网络访问和媒体 HTTP 播放。
- Emby 视频播放仍走应用内播放器，字幕识别沿用当前方案：通过设备麦克风提取视频声音，再调用 AI 接口翻译。
